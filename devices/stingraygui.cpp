...
StingrayGUI::StingrayGUI(const QSharedPointer<Core> &programm_core, QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::StingrayGUI)
{
    m_core = programm_core;
    ui->setupUi(this);
...
//    QObject::connect(m_core.data() , SIGNAL(UpdateStatus(QString, int)),this,
 //                    SLOT(ShowStatus(QString info,int color)));
    ...
    QObject::connect(m_core.data() , SIGNAL(UpdateData(Core::InputDataTypeDef)),this,
                     SLOT(ProcessReadedData(Core::InputDataTypeDef)));
...
void StingrayGUI::ProcessReadedData (Core::InputDataTypeDef data)
